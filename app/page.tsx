...
type Syllabus = { id: string; filename: string; size_bytes: number; page_count: number | null; created_at: string; status: string };
...
export default function Home() {
  ...
  function addFiles(files: FileList | File[]) {
    ...
    Array.from(files).forEach(file => {
      if (!/[.](pdf|html|htm)$/i.test(file.name)) errors.push(`${file.name}: choose a PDF or HTML file.`);
  ...
  async function openSyllabus(id: string) {
    ...
    try {
      ...
      else setMessage("Allow popups for this page to open the file.");
    } catch (error) { tab?.close(); setMessage(error instanceof Error ? error.message : "Could not open file."); }
  }

  return <div className="shell">
    <header><a className="brand" href="/"><img src="/grape.png" alt="" className="brandGrape" />bakatum<span className="brand-dot">.</span></a><span className="workspace">STUDY WORKSPACE</span>
    ...
    <main>
      <div className="intro">
        <div className="floatingGrapes" aria-hidden="true">
          <img src="/grape.png" alt="" className="grape grape1" />
          <img src="/grape.png" alt="" className="grape grape2" />
          <img src="/grape.png" alt="" className="grape grape3" />
          <img src="/grape.png" alt="" className="grape grape4" />
          <img src="/grape.png" alt="" className="grape grape5" />
          <img src="/grape.png" alt="" className="grape grape6" />
          <img src="/grape.png" alt="" className="grape grape7" />
          <img src="/grape.png" alt="" className="grape grape8" />
          <img src="/grape.png" alt="" className="grape grape9" />
          <img src="/grape.png" alt="" className="grape grape10" />
          <img src="/grape.png" alt="" className="grape grape11" />
          <img src="/grape.png" alt="" className="grape grape12" />
          <img src="/grape.png" alt="" className="grape grape13" />
          <img src="/grape.png" alt="" className="grape grape14" />
          <img src="/grape.png" alt="" className="grape grape15" />
          <img src="/grape.png" alt="" className="grape grape16" />
        </div>
        <div className="introContent">
          <span className="eyebrow">YOUR SEMESTER STARTS HERE</span><h1>Bring your courses<br />into one place.</h1><p>Add your syllabuses. One course or seven—there’s room for all of them.</p>
        </div>
      </div>
      <div className="layout">
        <section className="upload-panel" aria-labelledby="upload-title">
          <div className="section-heading"><div><span className="step">01 / IMPORT</span><h2 id="upload-title">Upload syllabuses</h2></div><span className="tag">PDF + HTML</span></div>
          <input ref={picker} className="sr-only" type="file" accept=".pdf,.html,.htm,application/pdf,text/html" multiple aria-label="Select PDF or HTML syllabuses" disabled={busy} onChange={event => { if (event.target.files) addFiles(event.target.files); event.target.value = ""; }} />
          <button className={`dropzone ${dragging ? "dragging" : ""}`} disabled={busy} onClick={() => picker.current?.click()} onDragOver={event => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); addFiles(event.dataTransfer.files); }}>
            <span className="upload-symbol" aria-hidden="true">↑</span><strong>Drop your syllabuses here</strong><span>or <u>browse files</u></span><small>Multiple files · Up to 20 MB each</small>
          </button>
          {message && <p className="notice error" role="alert">{message}</p>}
          {queue.length > 0 && <div className="queue" aria-live="polite">{queue.map(item => <div className="file-row" key={item.id}><span className="pdf-icon" aria-hidden="true">{/[.]pdf$/i.test(item.file.name) ? "PDF" : "HTML"}</span><div className="file-info"><strong>{item.file.name}</strong><span>{size(item.file.size)} · {item.status === "queued" ? "Ready to upload" : item.status === "uploading" ? "Uploading…" : item.status === "uploaded" ? "Saved" : "Upload failed"}</span>{item.error && <p className="file-error">{item.error}</p>}</div>{item.status !== "uploading" && <button className="remove" disabled={busy} aria-label={`Remove ${item.file.name} from queue`} onClick={() => setQueue(previous => previous.filter(q => q.id !== item.id))}>×</button>}</div>)}</div>}
          <div className="upload-footer"><span>{pending ? `${pending} file${pending === 1 ? "" : "s"} ready` : "No files waiting"}</span><button className="primary" disabled={!session || !pending || busy} onClick={upload}>{busy ? "Uploading…" : `Upload${pending ? ` ${pending} file${pending === 1 ? "" : "s"}` : " files"}`}</button></div>
          <p className="privacy">Your files stay private. Uploading does not send them to AI.</p>
        </section>
        <aside>
      ...
      <section className="saved-section" aria-labelledby="saved-title"><div className="saved-heading"><h2 id="saved-title">Your syllabuses</h2><span>{saved.length} saved</span></div>{loading ? <p role="status">Loading syllabuses…</p> : saved.length ? <div className="saved-list">{saved.map(item => <div className="file-row" key={item.id}><span className="pdf-icon" aria-hidden="true">{/[.]pdf$/i.test(item.filename) ? "PDF" : "HTML"}</span><div className="file-info"><strong>{item.filename}</strong><span>{item.page_count != null ? `${item.page_count} page${item.page_count === 1 ? "" : "s"} · ` : "HTML document · "}{size(item.size_bytes)} · {new Date(item.created_at).toLocaleDateString()}</span></div><button className="text-button" onClick={() => openSyllabus(item.id)}>{/[.]pdf$/i.test(item.filename) ? "Open PDF" : "Download HTML"}</button></div>)}</div> : <div className="empty"><span className="empty-line" /><p>No syllabuses yet.<br /><span>Your uploaded files will appear here.</span></p></div>}</section>
